...
function generateUnifiedTypeMappings(
  ...
  const rerankerModels = models
    .filter(m => hasCapability(getCleanModelName(m), 'reranker'))
    .map(m => `'${getCleanModelName(m)}'`)
    .join(' | ');
...
export type AiModelInputMap = {
  [K in AvailableModel]:
    ...
    K extends ${rerankerModels || 'never'} ? RerankerInput :
...
export type AiModelOutputMap = {
  [K in AvailableModel]:
    ...
    K extends ${rerankerModels || 'never'} ? RerankerOutput :
...
function generateModelCatalog(models: ApiModel[]): string {
  ...
  sortedModels.forEach(model => {
    ...
    // bge-reranker-base is a reranker model, not text-classification
    // It requires { query, contexts } input format, not { text }
    if (model.name === '@cf/baai/bge-reranker-base') {
      capabilities = ['reranker'];
    }

    // melotts is TTS - Cloudflare reports it incorrectly
    // Note: flux (@cf/deepgram/flux) is NOT TTS - it's a WebSocket streaming model for real-time audio
    // and cannot be tested via standard HTTP API (requires sample_rate, encoding, and WebSocket connection)
    if (model.name === '@cf/myshell-ai/melotts-mini-en') {
      capabilities = ['tts'];
    }

    // Handle multimodal models
    // Note: llava models are Image-to-Text only (vision), NOT chat models
    // llama-4-maverick-17b is a true multimodal chat model that supports vision
    if (model.name === 'llama-4-maverick-17b') {
      // Maverick supports both chat and vision
    ...
    } else if (model.name.includes('llava') || (model.name.includes('llama-3.2') && model.name.includes('vision'))) {
      // llava and llama-3.2 vision models are Image-to-Text only (vision capability)
      // Do NOT add 'chat' - they don't support standard chat completions
      // Replace capabilities with just 'vision'
      capabilities = ['vision'];
    } else if (model.name.includes('vision')) {
      // Other vision models - add vision capability if not already present
      if (!capabilities.includes('vision')) {
        capabilities.push('vision');
      }
    }
    ...
    // Skip flux-2-dev - its content filter is too aggressive and flags all prompts
    if (cleanName === 'flux-2-dev') {
      console.log(`⚠ Skipping flux-2-dev (content filter too aggressive for testing)`);
      return;
    }
...
async function loadModelRouterConfig(): Promise<{ models: ApiModel[], schemas: Record<string, ModelSchema> }> {
  try {
    ...
    uniqueModels.forEach(modelName => {
      ...
      if (modelName === 'glm-4.6-ludicrous' || modelName === 'glm-4.7-ludicrous') {
        rateLimits = {
...
/**
 * Checks if a model has a planned_deprecation_date property that is in the past
 */
function isModelDeprecated(model: ApiModel): boolean {
  const deprecationProperty = model.properties.find(p => p.property_id === 'planned_deprecation_date');
  if (!deprecationProperty || typeof deprecationProperty.value !== 'string') {
    return false;
  }

  const deprecationDate = new Date(deprecationProperty.value);
  const today = new Date();
  // Set time to midnight for date-only comparison
  today.setHours(0, 0, 0, 0);

  return deprecationDate < today;
}

/**
 * Filters out models that have passed their planned deprecation date
 */
function filterDeprecatedModels(models: ApiModel[]): { active: ApiModel[], deprecated: ApiModel[] } {
  const active: ApiModel[] = [];
  const deprecated: ApiModel[] = [];
...
async function generateAiTypeScript(): Promise<void> {
  try {
    ...
    const allCloudflareModels = await fetchModels();
    console.log(`Found ${allCloudflareModels.length} Cloudflare models`);

    // Filter out deprecated models
    const { active: cloudflareModels, deprecated: deprecatedModels } = filterDeprecatedModels(allCloudflareModels);
    if (deprecatedModels.length > 0) {
      console.log(`Filtered out ${deprecatedModels.length} deprecated models:`);
      deprecatedModels.forEach(m => {
        const depDate = m.properties.find(p => p.property_id === 'planned_deprecation_date')?.value;
        console.log(`  - ${m.name} (deprecated: ${depDate})`);
      });
    }
    console.log(`Active Cloudflare models: ${cloudflareModels.length}`);
