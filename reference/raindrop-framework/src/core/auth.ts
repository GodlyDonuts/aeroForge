 ...
 * Optionally validates audience via LM_AUTH_ALLOWED_AUDIENCES (comma-separated list).
 *
  ...
  if (authHeader && authHeader.startsWith('Bearer ')) {
    ...
    if (token) {
      try {
        ...
        const allowedAudiences = env.LM_AUTH_ALLOWED_AUDIENCES?.split(',').map(a => a.trim());
        ...
        // Verify the JWT against any of the trusted issuers (and audiences if configured)
