...
describe('auth', () => {
  ...
  describe('verifyIssuer', () => {
    ...
    it('should validate audience when LM_AUTH_ALLOWED_AUDIENCES is configured', async () => {
      const mockJwt = {
        payload: { sub: 'user123', iss: 'test-issuer', aud: 'my-client-id' },
        header: { alg: 'RS256' },
        protectedHeader: {},
      } as unknown as JWTVerifyResult;
      ...
      const env: {
        LM_AUTH_ALLOWED_ORIGINS: string;
        LM_AUTH_ALLOWED_ISSUERS: string;
        LM_AUTH_ALLOWED_AUDIENCES: string;
        mem: KvCache;
        logger: typeof mockLogger;
        jwt?: JWTVerifyResult;
      } = {
        LM_AUTH_ALLOWED_ORIGINS: 'localhost',
        LM_AUTH_ALLOWED_ISSUERS: 'test-issuer',
        LM_AUTH_ALLOWED_AUDIENCES: 'my-client-id',
        mem: mockKvCache,
        logger: mockLogger,
      };
    ...
    it('should handle multiple comma-separated audiences', async () => {
      const mockJwt = {
        payload: { sub: 'user123', iss: 'test-issuer', aud: 'audience2' },
        header: { alg: 'RS256' },
        protectedHeader: {},
      } as unknown as JWTVerifyResult;
      ...
      const env: {
        ...
        LM_AUTH_ALLOWED_ORIGINS: 'localhost',
        LM_AUTH_ALLOWED_ISSUERS: 'test-issuer',
        LM_AUTH_ALLOWED_AUDIENCES: 'audience1, audience2, audience3',
        mem: mockKvCache,
        logger: mockLogger,
      };
